...
export default function DoctorNavbar({ user }: DoctorNavbarProps) {
            ...
            <nav className={`fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-20">
                        {/* Logo */}
                        <Link href="/doctor/dashboard" className="flex-shrink-0 flex items-center gap-2 group cursor-pointer">
                            ...
                            <div className="relative pl-4 border-l border-slate-200">
                                ...
                                >
                                    <div className="hidden lg:block">
                                        <p className="text-sm font-bold text-slate-900 group-hover:text-teal-700 transition-colors">{user?.name || "Doctor"}</p>
                                        <p className="text-xs text-slate-500">ID: {user?.customId || "Pending"}</p>
                                    </div>
